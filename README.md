...
The project is now distributed as **one Macros.ahk` package**. When it starts, a small resolution selector asks which Destiny 2 resolution is being used.
...
On launch, `Macros.ahk` asks for the Destiny 2 resolution and loads the corresponding coordinate profile before enabling the macros.
...
4. Set up the four NetLimiter controls:
   ...
   - `F7` → `7500`
   - `F8` → `30000 / 30k`
5. Run `Macros.ahk` and select your Destiny 2 resolution when prompted.
