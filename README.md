...
The AHK scripts **do not communicate with NetLimiter directly**. They simply press the corresponding F5/F6/F7/F8 keys at the required points in their sequences. The F-key actions themselves therefore need to be configured separately in NetLimiter / the surrounding setup.
...
The `AllInOne.ahk` script combines all four macros into one file while keeping their individual hotkeys:
...
The **F5/F6/F7/F8 controls remain the same** because the combined script sends the exact same game / NetLimiter control keys used by the individual scripts.
...
4. Set up the four NetLimiter controls:
   ...
   - `F7` → `7500`
   - `F8` → `30000 / 30k`
5. Run the desired individual AHK script, or use `AllInOne.ahk` for all four macros.
...
- These macros are written around **one specific Destiny 2 configuration**.
- **2304x1440** is the reference resolution.
- The hard-coded mouse coordinates are not universal.
